...
from framework.timing import Timer, Timed, Timeout
...
'''
Backs off from the torpedoes board if the sub is too close
'''
class BackOff(Task):
    def on_first_run(self):
        self.target = (Torpedoes.camera_width/4, Torpedoes.camera_height/2)
        self.heading_target = HeadingTarget()
        self.pid_loop_x = PIDLoop()

    def on_run(self):
        print("backing off")
        vision_results = shm.torpedoes_results.get()
        board_coords = get_board_center(vision_results)
        self.heading_target.run(
                board_coords, px=0.02, 
                py=0.001, target=self.target, deadband=(60, 60),
                valid=Torpedoes.camera_valid)
        if self.pid_loop_x.finished:
            self.finish()
        else:
            self.pid_loop_x(
                input_value=vision_results.board_height,
                output_function=VelocityX(), 
                p=.001, i=0, d=0, target=Torpedoes.camera_height*0.7, 
                deadband=100)

'''
Rotates the sub in heading through a specified angle, by setting the heading
desire to a value of step_size more than the current heading
'''
class RotateHeading(Task):
    def on_first_run(self, delta_heading, step_size):
        self.heading_vel = RelativeToCurrentHeading(step_size)
        self.total_delta_heading = 0
        self.prev_heading = shm.kalman.heading.get()
        self.delta_heading_target = delta_heading

    def update_delta_heading(self):
        curr_heading = shm.kalman.heading.get()
        delta_heading = curr_heading - self.prev_heading

        # Catch the rollover from 359 to 0
        if abs(delta_heading) > 180:
            delta_heading = 359 - self.prev_heading + curr_heading

        self.total_delta_heading += delta_heading
        self.prev_heading = curr_heading

    def on_run(self, *args):
        self.update_delta_heading()
        self.heading_vel()

        if self.total_delta_heading >= self.delta_heading_target:
            self.finish()
...
class LocateBoard(Task):
    ...
    def on_run(self):
        ...
        if self.board_visible:
            if vision_results.board_height > 0.95 * Torpedoes.camera_height:
                self.back_off()
            elif heading_target_board.finished:
                self.found_board = True
                self.finish()
                self.logi("Finished LocateBoard")
            else:
                self.logi("Targeting board")
                # print(Torpedoes.camera_center)
                board_coords = get_board_center(vision_results)
                heading_target_board.run(board_coords, px=0.05, py=0.001, 
                    target=Torpedoes.camera_center, deadband=(60, 60), 
                    valid=Torpedoes.camera_valid)
