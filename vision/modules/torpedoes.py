...
class Torpedoes(ModuleBase):
    ...
    def num_edge_points(self, poly):
        # These lines cut off the corners of the camera, which are darkened
        # since the forecam enclosure on Thor is too small 
        lines = [((98, 0), (0, 95), True), ((1502, 0), (1600, 93), True), 
                 ((0, 840), (282, 1200), False), ((1600, 840), (1317, 1200), False)]
    ...
    def find_board(self, yellow_contours):
        ...
        # The largest four sided polygon contour is the torpedoes board
        # Heuristic to make lower contours preferred (to avoid reflections)
        ...
        if board_poly.shape[0] == 4:
            ...
            board_square = np.float32(
                [[0,0],[self.UNSKEWED_SIDE_LEN,0],
                [self.UNSKEWED_SIDE_LEN,self.UNSKEWED_SIDE_LEN],
                [0,self.UNSKEWED_SIDE_LEN]])
            board_skewed = np.float32(outline)

            unskew_mat = cv2.getPerspectiveTransform(board_skewed, board_square)
            unskewed = cv2.warpPerspective(
                self.mat, unskew_mat, 
                (self.UNSKEWED_SIDE_LEN, self.UNSKEWED_SIDE_LEN), 
